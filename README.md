...
**Live demo page (judges):** enable **GitHub Pages** from branch **`master`** / folder **`docs`**, then open **`https://cuongtranxuan.github.io/blockchain-skills-agent/`** — scenario demos are **HTML/CSS terminals** (sharp on any screen; cover + architecture still use PNGs with proper scaling). See **`SUBMISSION.md`** for image URLs and how to sync `videoURL` to Synthesis.
